...
class CompiledWaveletBank(torch.nn.Module):
    def __init__(self, wl_dict: List[str], dec_lev: int, img_size: Tuple[int, int], device: torch.device, dtype: torch.dtype, dirac: bool):
        super().__init__()
    ...
    def decompose_flat(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for lev in range(self.dec_lev):
            ...
            curr_x = F.pad(curr_x, (self.pad, self.pad, self.pad, self.pad), mode='reflect')
    ...
    def reconstruct_from_flat(self, flat_coeffs: torch.Tensor) -> torch.Tensor:
        ...
        for lev in range(self.dec_lev - 1, -1, -1):
            ...
            col_rec = F.conv_transpose2d(sub_bands, self.col_rec_filters, stride=(2, 1), groups=2 * self.num_bases)
            col_rec = col_rec[:, :, self.pad : self.pad + level_h, :]
            
            # Row Transpose Pass (Up-sample horizontally)
            row_rec = F.conv_transpose2d(col_rec, self.row_rec_filters, stride=(1, 2), groups=self.num_bases)
            curr_LL = row_rec[:, :, :, self.pad : self.pad + level_w]
