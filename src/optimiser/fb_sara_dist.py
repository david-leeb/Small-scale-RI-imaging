...
from .forward_backward_dist import ForwardBackward
...
class FBSARADist(ForwardBackward):
    """
    ...
    def __init__(
        ...
        """
        super(FBSARADist, self).__init__(
            meas,
    ...
    def initialisation(self) -> None:
        ...
        # step size
        op_norm = self._meas_op_precise.get_op_norm()
        if is_root:
            self._gd_step_size = 1.98 / op_norm
        ...
        if self._new_heu:
            ...
            adj = self._meas_op.adjoint_op(noise)
            if is_root:
                self._heuristic = (adj / self._meas_op.get_psf().max()).std().item() / self._meas_bp.max().item()
                if self._verbose:
                    print(
            ...
            if not self._use_ROP:
                op_norm_prime = self._meas_op_precise.get_op_norm_prime()
            if is_root:
                self._heuristic = 1 / np.sqrt(2 * op_norm)
                
                if self._verbose:
                    print(f"INFO: measurement operator norm {op_norm}", flush=True)
                    print(f"INFO: heuristic noise level: {self._heuristic}", flush=True)
                
                if not self._use_ROP and not np.isclose(np.sqrt(op_norm_prime / op_norm), 1.0):
                    heu_corr_factor = np.sqrt(op_norm_prime / op_norm)
        ...
        if is_root:     
            ...
            if self._verbose:
                print(
                    f"INFO: estimated noise floor level in wavelet coefficients: {self._heuristic/3.0}"
                )
            ...
            if self._verbose:
                ...
                print(f"INFO: heuristic soft thresholding level: {self._heuristic/3.0}")
    ...
    @torch.no_grad()
    def _each_iter_end(self, residual=None) -> None:
        """
        ...
        if self._reweighting_flag:
            ...
            if self._verbose:
                print(
                    ...
                    f"{np.std(residual/self._psf_peak).item()}",
                    flush=True,
                )
            ...
            if self._reweight_save:
                ...
                fits.writeto(
                    ...
                    residual.astype(np.float32) / self._psf_peak,
                    overwrite=True,
                )
    ...
    def finalisation(self) -> None:
        ...
        is_root = self.rank == 0
        residual = self.get_residual_image()
        ...
        if self._use_ROP:
            ...
            for sub_ops in self._meas_op._wstack_meas_ops:
                sub_ops._data_weight /= self._weight_correction
            ...
            if is_root:
                ...
                x_out = torch.zeros(1, 1, *self._meas_op._img_size, device=self._meas_op.device, dtype=self._meas_op._dtype)
            
            
            dirty = self._meas_op.adjoint_classical(self._y_uncompressed)
            model_bp = self._meas_op.forward_adjoint_op(x_out)
            psf_classical = self._meas_op.get_psf()
            ...
            if is_root:
                ...
                psf_peak_classical = psf_classical.max().item()
                print(
